...
- Serial limits copied from firmware: command line ≤ 255 bytes (`CONFIG_SHELL_CMD_BUFF_SIZE=256`), joined prompt ≤ 191 bytes (`cmd_gen`'s `char prompt[192]`). Overflow is a client-side error, never truncation.
...
- Produces: `SHELL_CMD_MAX = 255`, `PROMPT_BUF_MAX = 191`, `build_gen_command(prompt: str, opts: GenOpts, seed: int) -> str`. Task 5 consumes `build_gen_command`.
