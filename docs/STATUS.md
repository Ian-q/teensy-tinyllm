...
| **Logits are bit-identical across architectures** | Required before a host can be the far end of a Semaphore decode, and **not true today**. `tq_forward` calls `expf`/`sinf`/`cosf`/`powf`; newlib and macOS libm disagree in the last ulp, and `-ffp-contract=fast` fuses multiply-adds differently per target. One ulp desynchronises the arithmetic decoder and destroys the whole message. Two boards running the identical binary agree by construction, so device-to-device is unaffected. Everything below the logits — the count tables, the `exp2` — is already integer-only for exactly this reason. |
